...
const auth=(req,res,next)=>req.get('X-IG-API-KEY')===cfg.apiKey?next():res.status(401).json({errorCode:'invalid-api-key'});
async function igSession(){if(!session||Date.now()-session.startedAt>14400000)await login();return session;}
const igHeaders=(s,v='1')=>({'X-IG-API-KEY':cfg.apiKey,CST:s.cst,'X-SECURITY-TOKEN':s.xst,Version:v,Accept:'application/json','Content-Type':'application/json'});
let btcMarket=null;
async function findBitcoin(){
 const s=await igSession();
 const r=await fetch(`${cfg.base}/markets?searchTerm=Bitcoin`,{headers:igHeaders(s)});
 const b=await r.json().catch(()=>({}));
 if(!r.ok||!Array.isArray(b.markets))throw new Error(b.errorCode||`search-${r.status}`);
 const list=b.markets.filter(m=>/bitcoin/i.test(`${m.instrumentName||''} ${m.epic||''}`));
 const m=list.find(x=>x.marketStatus==='TRADEABLE')||list[0];
...
app.post('/ig/demo/bitcoin-order',auth,async(req,res)=>{
 try{
  ...
  const size=Number(req.body?.size??0.1);
  ...
  const p={currencyCode:'EUR',direction,epic:market.epic,expiry:'-',forceOpen:true,guaranteedStop:false,orderType:'MARKET',size,trailingStop:false};
  const sl=Number(req.body?.stopLevel),tp=Number(req.body?.limitLevel);
  if(Number.isFinite(sl))p.stopLevel=sl;if(Number.isFinite(tp))p.limitLevel=tp;
  const s=await igSession();
  const r=await fetch(`${cfg.base}/positions/otc`,{method:'POST',headers:igHeaders(s,'2'),body:JSON.stringify(p)});
  const b=await r.json().catch(()=>({}));
  res.status(r.status).json({mode:'IG_DEMO_ONLY',epic:market.epic,...b});
